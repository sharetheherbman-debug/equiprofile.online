...
const requiredVars = ["DATABASE_URL", "JWT_SECRET", "ADMIN_UNLOCK_PASSWORD"];
...
if (process.env.NODE_ENV === "production") {
  ...
  const dangerousDefaults = [
    {
      var: "JWT_SECRET",
      badValues: ["your_jwt_secret_here", "FHnuavgCmZtlXQ2AAxgq+bmpt6D4Iqfl"],
    },
    {
      var: "ADMIN_UNLOCK_PASSWORD",
      badValues: ["ashmor12@", "EquiProfile2026!Admin"],
    },
  ];
  ...
  if (insecure.length > 0) {
    console.error(
      "❌ SECURITY ERROR: Default values detected in production!\n",
    );
...
console.log("\n📦 Features:");
console.log(
  `   Stripe: ${process.env.ENABLE_STRIPE === "true" ? "✅ enabled" : "❌ disabled"}`,
);
console.log(
  `   Uploads: ${process.env.ENABLE_UPLOADS === "true" ? "✅ enabled" : "❌ disabled"}`,
);
console.log(
  `   OAuth: ${process.env.OAUTH_SERVER_URL ? "✅ configured" : "❌ not configured"}`,
);
console.log(
  `   PWA: ${process.env.VITE_PWA_ENABLED === "true" ? "✅ enabled" : "❌ disabled"}`,
);
