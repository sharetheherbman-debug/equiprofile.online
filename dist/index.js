...
var users, horses2, healthRecords, trainingSessions, feedingPlans, documents, weatherLogs, systemSettings, adminSessions, adminUnlockAttempts, activityLogs, backupLogs, stables, stableMembers, stableInvites, events, eventReminders, feedCosts, vaccinations, dewormings, shareLinks, competitions, documentTags, stripeEvents, messageThreads, messages, competitionResults, trainingProgramTemplates, trainingPrograms, reports, reportSchedules, breeding, foals, pedigree, lessonBookings, trainerAvailability, apiKeys, webhooks;
...
async function createHorse(data) {
  ...
  const result = await db.insert(horses2).values(data);
  return result[0].insertId;
}
async function getHorsesByUserId(userId) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(horses2).where(
    and(eq(horses2.userId, userId), eq(horses2.isActive, true))
  ).orderBy(desc(horses2.createdAt));
}
async function getHorseById(id, userId) {
  const db = await getDb();
  if (!db) return void 0;
  const result = await db.select().from(horses2).where(
    and(eq(horses2.id, id), eq(horses2.userId, userId))
  ).limit(1);
  return result.length > 0 ? result[0] : void 0;
}
async function updateHorse(id, userId, data) {
  const db = await getDb();
  if (!db) return;
  await db.update(horses2).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(
    and(eq(horses2.id, id), eq(horses2.userId, userId))
  );
}
async function deleteHorse(id, userId) {
  const db = await getDb();
  if (!db) return;
  await db.update(horses2).set({ isActive: false, updatedAt: /* @__PURE__ */ new Date() }).where(
    and(eq(horses2.id, id), eq(horses2.userId, userId))
  );
}
...
function generateCSVFilename(prefix) {
  ...
  return `${prefix}_${timestamp2}.csv`;
}
function exportHorsesCSV(horses3) {
  const headers = [
    "id",
    "name",
    "breed",
...
function exportHealthRecordsCSV(records) {
  const headers = [
    "id",
    "horseId",
    "horseName",
...
function exportTrainingSessionsCSV(sessions) {
  const headers = [
    "id",
    "horseId",
    "horseName",
...
function exportCompetitionsCSV(competitions2) {
  const headers = [
    "id",
    "horseId",
    "horseName",
  ...
  const data = competitions2.map((comp) => ({
    ...comp,
...
function exportFeedCostsCSV(costs) {
  const headers = [
    "id",
    "horseId",
    "horseName",
...
function exportDocumentsCSV(documents2) {
  const headers = [
    "id",
    "horseId",
    "horseName",
...
init_env();
init_schema();
import { eq as eq2, and as and2, desc as desc2, sql as sql2, gte as gte2, lte as lte2, or as or2, inArray } from "drizzle-orm";
...
var appRouter = router3({
  ...
  user: router3({
    ...
    getDashboardStats: subscribedProcedure.query(async ({ ctx }) => {
      const horses3 = await getHorsesByUserId(ctx.user.id);
  ...
  horses: router3({
    ...
    exportCSV: subscribedProcedure.query(async ({ ctx }) => {
      const horses3 = await getHorsesByUserId(ctx.user.id);
      const csv = exportHorsesCSV(horses3);
  ...
  healthRecords: router3({
    ...
    exportCSV: subscribedProcedure.query(async ({ ctx }) => {
      const records = await getHealthRecordsByUserId(ctx.user.id);
  ...
  feeding: router3({
    ...
    exportCSV: subscribedProcedure.query(async ({ ctx }) => {
      const feedPlans = await getFeedingPlansByUserId(ctx.user.id);
  ...
  documents: router3({
    ...
    exportCSV: subscribedProcedure.query(async ({ ctx }) => {
      const documents2 = await getDocumentsByUserId(ctx.user.id);
      const csv = exportDocumentsCSV(documents2);
...
Format your response as JSON with keys: recommendation, explanation, precautions, bestTime`;
  ...
  admin: router3({
    ...
    getUserDetails: adminUnlockedProcedure.input(z2.object({ userId: z2.number() })).query(async ({ input }) => {
      ...
      const horses3 = await getHorsesByUserId(input.userId);
      const activity = await getUserActivityLogs(input.userId, 20);
      return { user, horses: horses3, activity };
    }),
  ...
  trainingPrograms: router3({
    ...
    getTemplate: protectedProcedure.input(z2.object({ id: z2.number() })).query(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) return null;
      const templates = await db.select().from(trainingProgramTemplates).where(eq2(trainingProgramTemplates.id, input.id)).limit(1);
    ...
    updateTemplate: subscribedProcedure.input(z2.object({
      id: z2.number(),
      ...
      level: z2.string().optional(),
      goals: z2.string().optional(),
      programData: z2.string().optional(),
      isPublic: z2.boolean().optional()
    })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError4({ code: "INTERNAL_SERVER_ERROR" });
      const { id, ...updateData } = input;
      const existing = await db.select().from(trainingProgramTemplates).where(eq2(trainingProgramTemplates.id, id)).limit(1);
      if (existing.length === 0 || existing[0].userId !== ctx.user.id) {
        throw new TRPCError4({ code: "FORBIDDEN" });
      }
      await db.update(trainingProgramTemplates).set(updateData).where(eq2(trainingProgramTemplates.id, id));
      return { success: true };
    }),
    deleteTemplate: subscribedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError4({ code: "INTERNAL_SERVER_ERROR" });
      const existing = await db.select().from(trainingProgramTemplates).where(eq2(trainingProgramTemplates.id, input.id)).limit(1);
      if (existing.length === 0 || existing[0].userId !== ctx.user.id) {
        throw new TRPCError4({ code: "FORBIDDEN" });
      }
      await db.delete(trainingProgramTemplates).where(eq2(trainingProgramTemplates.id, input.id));
      return { success: true };
    }),
    duplicateTemplate: subscribedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError4({ code: "INTERNAL_SERVER_ERROR" });
      const existing = await db.select().from(trainingProgramTemplates).where(eq2(trainingProgramTemplates.id, input.id)).limit(1);
  ...
  breeding: router3({
    ...
    list: protectedProcedure.input(z2.object({
      ...
      const db = await getDb();
      if (!db) return [];
      const userHorses = await db.select({ id: horses.id }).from(horses).where(eq2(horses.userId, ctx.user.id));
      const horseIds = userHorses.map((h) => h.id);
      if (horseIds.length === 0) return [];
      let query = db.select().from(breeding).where(inArray(breeding.mareId, horseIds));
      if (input.mareId) {
        query = db.select().from(breeding).where(and2(
          inArray(breeding.mareId, horseIds),
          eq2(breeding.mareId, input.mareId)
        ));
      }
    ...
    get: protectedProcedure.input(z2.object({ id: z2.number() })).query(async ({ ctx, input }) => {
      ...
      const userHorses = await db.select({ id: horses.id }).from(horses).where(eq2(horses.userId, ctx.user.id));
      const horseIds = userHorses.map((h) => h.id);
      const records = await db.select().from(breeding).where(and2(
        eq2(breeding.id, input.id),
        inArray(breeding.mareId, horseIds)
      )).limit(1);
    ...
    update: subscribedProcedure.input(z2.object({
      id: z2.number(),
      stallionName: z2.string().optional(),
      breedingDate: z2.string().optional(),
      method: z2.enum(["natural", "artificial", "embryo_transfer"]).optional(),
      veterinarianName: z2.string().optional(),
      cost: z2.number().optional(),
      notes: z2.string().optional()
    })).mutation(async ({ ctx, input }) => {
      ...
      const userHorses = await db.select({ id: horses.id }).from(horses).where(eq2(horses.userId, ctx.user.id));
      const horseIds = userHorses.map((h) => h.id);
      const existing = await db.select().from(breeding).where(and2(
        eq2(breeding.id, id),
        inArray(breeding.mareId, horseIds)
      )).limit(1);
      if (existing.length === 0) {
      ...
      await db.update(breeding).set(dataToUpdate).where(eq2(breeding.id, id));
      return { success: true };
    }),
    delete: subscribedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      ...
      await db.delete(breeding).where(eq2(breeding.id, input.id));
      return { success: true };
    }),
    confirmPregnancy: subscribedProcedure.input(z2.object({
      id: z2.number(),
      confirmed: z2.boolean(),
      confirmationDate: z2.string().optional(),
      ...
      await db.update(breeding).set(updateData).where(eq2(breeding.id, input.id));
      return { success: true };
    }),
    ...
    listFoals: protectedProcedure.input(z2.object({
      breedingId: z2.number().optional()
    })).query(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) return [];
      if (input.breedingId) {
        return db.select().from(foals).where(eq2(foals.breedingId, input.breedingId)).orderBy(desc2(foals.birthDate));
      }
      return db.select().from(foals).orderBy(desc2(foals.birthDate));
    }),
    exportCSV: subscribedProcedure.query(async ({ ctx }) => {
      const dbInstance = await getDb();
      if (!dbInstance) {
      ...
      const breedingRecords = await dbInstance.select().from(breeding).where(eq2(breeding.userId, ctx.user.id)).orderBy(desc2(breeding.createdAt));
      ...
      const csv = data.length > 0 ? [headers.join(","), ...data.map((row) => headers.map((h) => row[h]).join(","))].join("\n") : headers.join(",");
  ...
  trainerAvailability: router3({
    ...
    list: protectedProcedure.query(async ({ ctx }) => {
      const db = await getDb();
      if (!db) return [];
      return db.select().from(trainerAvailability).where(and2(
        eq2(trainerAvailability.trainerId, ctx.user.id),
        eq2(trainerAvailability.isActive, true)
    ...
    update: protectedProcedure.input(z2.object({
      ...
      const existing = await db.select().from(trainerAvailability).where(eq2(trainerAvailability.id, input.id)).limit(1);
      ...
      await db.update(trainerAvailability).set(updateData).where(eq2(trainerAvailability.id, input.id));
      return { success: true };
    }),
    delete: protectedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
      ...
      await db.update(trainerAvailability).set({ isActive: false }).where(eq2(trainerAvailability.id, input.id));
      return { success: true };
    })
  }),
  // Lesson bookings management
  lessonBookings: router3({
    create: protectedProcedure.input(z2.object({
      trainerId: z2.number(),
      horseId: z2.number().optional(),
      lessonDate: z2.string(),
      duration: z2.number(),
      lessonType: z2.string().optional(),
      location: z2.string().optional(),
    ...
    list: protectedProcedure.input(z2.object({
      asTrainer: z2.boolean().optional(),
      status: z2.enum(["scheduled", "completed", "cancelled", "no_show"]).optional()
    })).query(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) return [];
      let conditions = [];
      if (input.asTrainer) {
        conditions.push(eq2(lessonBookings.trainerId, ctx.user.id));
      } else {
        conditions.push(eq2(lessonBookings.clientId, ctx.user.id));
      }
      if (input.status) {
        conditions.push(eq2(lessonBookings.status, input.status));
      }
      return db.select().from(lessonBookings).where(and2(...conditions)).orderBy(desc2(lessonBookings.lessonDate));
    }),
    get: protectedProcedure.input(z2.object({ id: z2.number() })).query(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) {
      ...
      const lessons = await db.select().from(lessonBookings).where(eq2(lessonBookings.id, input.id)).limit(1);
      if (!lessons.length) {
        throw new TRPCError4({ code: "NOT_FOUND" });
      }
    ...
    update: protectedProcedure.input(z2.object({
      id: z2.number(),
      lessonDate: z2.string().optional(),
      duration: z2.number().optional(),
      lessonType: z2.string().optional(),
      location: z2.string().optional(),
      status: z2.enum(["scheduled", "completed", "cancelled", "no_show"]).optional(),
      fee: z2.number().optional(),
      paid: z2.boolean().optional(),
      notes: z2.string().optional()
    })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) {
      ...
      const existing = await db.select().from(lessonBookings).where(eq2(lessonBookings.id, input.id)).limit(1);
      if (!existing.length) {
        throw new TRPCError4({ code: "NOT_FOUND" });
      }
      ...
      if (input.fee !== void 0) updateData.fee = input.fee;
      if (input.paid !== void 0) updateData.paid = input.paid;
      if (input.notes) updateData.notes = input.notes;
      await db.update(lessonBookings).set(updateData).where(eq2(lessonBookings.id, input.id));
      return { success: true };
    }),
    delete: protectedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      ...
      await db.delete(lessonBookings).where(eq2(lessonBookings.id, input.id));
      return { success: true };
    }),
    markCompleted: protectedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) {
        throw new TRPCError4({ code: "INTERNAL_SERVER_ERROR" });
      }
      ...
      await db.update(lessonBookings).set({ status: "completed" }).where(eq2(lessonBookings.id, input.id));
      return { success: true };
    }),
    markCancelled: protectedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) {
        throw new TRPCError4({ code: "INTERNAL_SERVER_ERROR" });
      }
      ...
      await db.update(lessonBookings).set({ status: "cancelled" }).where(eq2(lessonBookings.id, input.id));
      return { success: true };
    })
  })
});
...
async function verifyApiKey2(req, res, next) {
...
apiRouter.use(verifyApiKey2);
apiRouter.get("/horses", async (req, res) => {
  try {
    ...
    const userHorses = await dbInstance.select().from(horses2).where(eq3(horses2.userId, userId)).orderBy(desc3(horses2.createdAt));
    res.json({
      success: true,
      data: userHorses,
...
apiRouter.get("/horses/:id", async (req, res) => {
  try {
    const userId = req.apiUserId;
    ...
    const horse = await dbInstance.select().from(horses2).where(eq3(horses2.id, horseId)).limit(1);
    if (!horse.length) {
...
apiRouter.get("/health-records/:horseId", async (req, res) => {
  try {
    const userId = req.apiUserId;
    ...
    const records = await dbInstance.select().from(healthRecords).where(eq3(healthRecords.horseId, horseId)).orderBy(desc3(healthRecords.recordDate));
    res.json({
      success: true,
      data: records,
...
apiRouter.get("/training-sessions/:horseId", async (req, res) => {
  try {
    const userId = req.apiUserId;
    const horseId = parseInt(req.params.horseId);
    if (isNaN(horseId)) {
    ...
    const sessions = await dbInstance.select().from(trainingSessions).where(eq3(trainingSessions.horseId, horseId)).orderBy(desc3(trainingSessions.sessionDate));
    res.json({
      success: true,
      data: sessions,
...
apiRouter.get("/competitions/:horseId", async (req, res) => {
  try {
    const userId = req.apiUserId;
    const horseId = parseInt(req.params.horseId);
    if (isNaN(horseId)) {
    ...
    const comps = await dbInstance.select().from(competitions).where(eq3(competitions.horseId, horseId)).orderBy(desc3(competitions.date));
    res.json({
      success: true,
      data: comps,
      count: comps.length
