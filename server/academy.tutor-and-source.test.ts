...
describe("Academy curriculum and Tutor boundaries", () => {
  ...
  it("uses original Academy Tutor framing and a server-resolved lesson context", () => {
    ...
    expect(router).toContain("lessonSlug: z.string().min(1).max(150).optional()");
    expect(router).toContain("getPublishedLessonBySlug(input.lessonSlug)");
    expect(router).toContain("Never claim a learner has completed a lesson or passed a competency");
    expect(router).toContain("contact a veterinarian or emergency professional promptly");
  });
  ...
  it("documents source-retirement without learner-history deletion", () => {
    ...
    expect(pipeline).toContain("never deletes rows or learner-completion history");
  });
});
