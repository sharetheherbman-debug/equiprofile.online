...
import { LESSON_PATHWAYS, LESSON_UNITS, type LessonUnitData } from "../lessonContent";
...
export async function syncAcademyCurriculum(): Promise<AcademyCurriculumSyncResult> {
  ...
  for (const lesson of LESSON_UNITS) {
    ...
    const currentIndex = pathwayLessons.findIndex((item) => item.slug === lesson.slug);
...
export async function getPublishedLessonBySlug(slug: string) {
  const db = await getDb();
  if (!db) throw new Error("Academy lesson lookup requires a database connection");
...
export function parseAcademyJson<T>(raw: string | null | undefined, fallback: T): T {
