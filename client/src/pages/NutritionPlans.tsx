...
function NutritionPlansContent() {
  ...
  useRealtimeModule('nutritionPlans', (action, data) => {
    if (action === 'created') {
      ...
      setLocalPlans((prev) => prev.map((plan) => (plan.id === data.id ? { ...plan, ...data } : plan)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        targetBodyCondition: formData.targetBodyCondition ? parseInt(formData.targetBodyCondition) : undefined,
        dailyFeedAmount: formData.dailyFeedAmount || undefined,
        feedingSchedule: formData.feedingSchedule || undefined,
        dailyCalories: formData.dailyCalories ? parseFloat(formData.dailyCalories) : undefined,
        dailyProtein: formData.dailyProtein ? parseFloat(formData.dailyProtein) : undefined,
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingPlan ? 'Edit' : 'Add'} Nutrition Plan</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="targetBodyCondition">Target Body Condition (1-9)</Label>
                  <Select value={formData.targetBodyCondition} onValueChange={(value) => setFormData({ ...formData, targetBodyCondition: value })}>
                    <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, dailyFeedAmount: e.target.value })}
                  placeholder="e.g., 3 kg pellets, 2 bales hay"
                />
              </div>

              <div>
                <Label htmlFor="feedingSchedule">Feeding Schedule (JSON format)</Label>
                <Textarea
                ...
                {activePlans.map((plan: any) => (
                  <Card key={plan.id}>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-lg font-medium">{plan.planName}</CardTitle>
                      <div className="flex gap-2">
                        <Badge variant="default">Active</Badge>
                        {plan.targetBodyCondition && <Badge variant="outline">Target BCS: {plan.targetBodyCondition}</Badge>}
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        <p className="text-sm"><span className="font-medium">Horse:</span> {horses?.find((h: any) => h.id === plan.horseId)?.name || 'Unknown'}</p>
                        {plan.description && <p className="text-sm text-muted-foreground">{plan.description}</p>}
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                          <div><span className="font-medium">Start:</span> {plan.startDate}</div>
                          {plan.endDate && <div><span className="font-medium">End:</span> {plan.endDate}</div>}
                          {plan.targetWeight && <div><span className="font-medium">Target Weight:</span> {plan.targetWeight} kg</div>}
                          {plan.dailyCalories && <div><span className="font-medium">Calories:</span> {plan.dailyCalories}</div>}
                          {plan.dailyProtein && <div><span className="font-medium">Protein:</span> {plan.dailyProtein}g</div>}
                          {plan.dailyFeedAmount && <div><span className="font-medium">Daily Feed:</span> {plan.dailyFeedAmount}</div>}
                        </div>
                        {plan.notes && <p className="mt-2 text-sm text-muted-foreground italic">{plan.notes}</p>}
                      </div>
                    </CardContent>
                  </Card>
                ))}
